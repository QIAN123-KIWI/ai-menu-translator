...
export default function AIMenuTranslator() {
    ...
    // 移除了 Fetch User Status 的 useEffect 钩子
    ...
    const groupItemsByCategory = (items) => {
        ...
        // 不会出现多个重复的分类标签（如多个“主菜”）
        const grouped = items.reduce((acc, item) => {
            // 使用 category 字段进行分组，如果缺失则归为“未分类”
            const category = item.category && item.category.trim() !== '' ? item.category : '未分类'; // 移除英文 fallback
            if (!acc[category]) {
                acc[category] = [];
            }
		...
		for (let attempt = 0; attempt < maxRetries; attempt++) {
			...
			try {
				...
				try {
					...
					if (firstBracket === -1) {
						console.error("Gemini Raw Text (no valid array found):", text);
					...
					try {
						...
						lastError = e2;
						console.error("Gemini Raw Text (Final Parse Failed):", jsonString); 
	...
	const handleFileSelect = async (e) => {
		...
		try {
			...
			setToastMessage("识别失败，请确保图片清晰，网络畅通。");
		} finally {
			setIsScanning(false);
			setScanStep('');
	...
	// --- Render ---
	const categorizedItems = groupItemsByCategory(menuItems); // Grouped and sorted items
    const scanSessions = groupItemsBySession(menuItems); // Grouped items by scan time
